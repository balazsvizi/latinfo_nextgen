...
// Alapértelmezett címzett: bejelentkezett admin e-mailje (DB, különben session) – az űrlapon szerkeszthető
$admin_email = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($config_id <= 0) {
        ...
        if ($eredmeny['ok']) {
            $siker = 'A teszt e-mail elküldve ide: ' . $cim . '.';
        } else {
...
$cimzett_ertek = ($_SERVER['REQUEST_METHOD'] === 'POST')
    ? trim($_POST['cim'] ?? '')
    : $admin_email;
?>
<div class="card card-narrow">
    <h2>Teszt e-mail küldése</h2>
    <p class="text-muted">A kiválasztott SMTP fiók nevében kiküld egy teszt levelet. A címzett mező <strong>előre kitöltve</strong> a bejelentkezett fiók e-mail címével, de <strong>szabadon módosítható</strong>.</p>
    ...
    <form method="post" id="email-teszt-form">
        ...
        <div class="form-group">
            <label for="email-teszt-cim">Címzett e-mail *</label>
            <input type="email" id="email-teszt-cim" name="cim" value="<?= h($cimzett_ertek) ?>" required placeholder="pelda@email.hu" autocomplete="email">
            <?php if ($admin_email === '' && $admin_id > 0): ?>
                <p class="text-muted" style="margin:0.35rem 0 0; font-size:0.875rem;">Nincs e-mail a profilodon – írj be címet, vagy <a href="<?= h(nextgen_url('admin/adminok/szerkeszt.php?id=')) ?><?= (int) $admin_id ?>">add meg az admin adatlapodon</a> a későbbi előtöltéshez.</p>
            <?php endif; ?>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn btn-primary" id="email-teszt-btn">Teszt e-mail küldése</button>
            <a href="<?= h(nextgen_url('admin/email/')) ?>" class="btn btn-secondary">Vissza</a>
        </div>
        <p id="email-teszt-ujratoltes" class="text-muted" style="margin-top:0.75rem;display:none;">Ha a küldés lefutott vagy timeoutolt, <a href="<?= h(nextgen_url('admin/email/teszt.php')) ?>">kattints ide</a> az oldal frissítéséhez.</p>
    </form>
    <script>
    (function() {
