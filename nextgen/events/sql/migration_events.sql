...
CREATE TABLE IF NOT EXISTS `events_calendar_events` (
    ...
    `event_change_active` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1 = aktív változás/elmaradás jelzés',
    `event_change_type` VARCHAR(20) NULL DEFAULT NULL COMMENT 'cancelled | modified',
    `event_change_note` TEXT NULL DEFAULT NULL COMMENT 'Publikus megjegyzés',
...
-- Kategóriák (hierarchikus, egyéni színnel)
...
-- Címkék + speciális csoportok + esemény–tag
