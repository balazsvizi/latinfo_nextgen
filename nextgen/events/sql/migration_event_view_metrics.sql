-- Esemény metrikák: oldalmegtekintés vs. naptár előnézet vs. további info kattintás + forrás (direct, calendar, list, …).
-- Futtatás: events_calendar_event_views tábla létezik (migration_events.sql).

ALTER TABLE `events_calendar_event_views`
    ADD COLUMN `metric_type` VARCHAR(32) NOT NULL DEFAULT 'page_view' COMMENT 'page_view | calendar_preview | external_info_click' AFTER `ip_hash`,
