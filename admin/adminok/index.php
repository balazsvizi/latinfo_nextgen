...
<div class="card">
    ...
    <form method="get" class="toolbar">
        ...
        <a href="<?= h(BASE_URL) ?>/admin/adminok/letrehoz.php" class="btn btn-primary">Új admin</a>
    </form>
        ...
        <table class="sortable-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="actions">
                        <a href="<?= h(BASE_URL) ?>/admin/adminok/szerkeszt.php?id=<?= (int)$a['id'] ?>" class="btn btn-sm btn-secondary">Szerkeszt</a>
                        <?php if ((int)$a['id'] !== (int)($_SESSION['admin_id'] ?? 0)): ?>
                            <?php if ($a['aktív']): ?>
                                <a href="<?= h(BASE_URL) ?>/admin/adminok/letilt.php?id=<?= (int)$a['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Letiltja ezt az admint?');">Letiltás</a>
                            <?php else: ?>
                                <a href="<?= h(BASE_URL) ?>/admin/adminok/engedelyez.php?id=<?= (int)$a['id'] ?>" class="btn btn-sm btn-primary">Engedélyezés</a>
                            <?php endif; ?>
                        <?php else: ?>
