...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['megjegyzes_szoveg'])) {
    ...
    if ($szoveg !== '') {
        ...
        redirect(BASE_URL . '/organizers/megtekint.php?id=' . $id . '#megjegyzesek');
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['log_esemeny'])) {
    ...
    if ($esemeny !== '') {
        ...
        redirect(BASE_URL . '/organizers/megtekint.php?id=' . $id . '#log');
    }
}
...
<div class="card card-szervezo-fejlec" id="megjegyzesek">
    <div class="szervezo-fejlec-sor">
        <div class="szervezo-nev-blokk">
            ...
            <p>
                <a href="<?= h(BASE_URL) ?>/organizers/szerkeszt.php?id=<?= $id ?>" class="btn btn-primary">Szerkesztés</a>
                <a href="<?= h(BASE_URL) ?>/organizers/" class="btn btn-secondary">← Lista</a>
            </p>
...
<div class="card card-collapsible-list" id="card-szamlazando">
    <h2>Számlázandó</h2>
    <p><a href="<?= h(BASE_URL) ?>/finance/szamlazando/letrehoz.php?szervezo_id=<?= $id ?>" class="btn btn-primary btn-sm">Új számlázandó</a></p>
            ...
            <tr class="<?= $i >= $show_limit ? 'collapse-extra' : '' ?>">
                ...
                <td class="actions">
                    <?php if (empty($sz['számla_id'])): ?>
                        <a href="<?= h(BASE_URL) ?>/finance/szamlak/letrehoz.php?szervezo_id=<?= (int)$id ?>&szamlazando_id=<?= (int)$sz['id'] ?>" class="btn btn-primary btn-sm">Számlakészítés</a>
                    <?php endif; ?>
                    <a href="<?= h(BASE_URL) ?>/finance/szamlazando/szerkeszt.php?id=<?= (int)$sz['id'] ?>" class="btn btn-sm btn-secondary">Szerkeszt</a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
<div class="card card-collapsible-list" id="card-szamlak">
    <h2>Számlák</h2>
    <p><a href="<?= h(BASE_URL) ?>/finance/szamlak/letrehoz.php?szervezo_id=<?= $id ?>" class="btn btn-primary btn-sm">Új számla</a></p>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr class="<?= $i >= $show_limit ? 'collapse-extra' : '' ?>">
                <td><?= (int)$sz['id'] ?></td>
                <td><a href="<?= h(BASE_URL) ?>/finance/szamlak/szerkeszt.php?id=<?= (int)$sz['id'] ?>&vissza=szervezo&szervezo_id=<?= (int)$id ?>"><?= h($sz['számla_szám']) ?></a></td>
                <td><?= h($sz['dátum']) ?></td>
                <td><?= number_format((float)$sz['összeg'], 0, ',', ' ') ?></td>
                <td><?= szamla_statusz_label($sz['státusz']) ?></td>
                <td><a href="<?= h(BASE_URL) ?>/finance/szamlak/szerkeszt.php?id=<?= (int)$sz['id'] ?>&vissza=szervezo&szervezo_id=<?= (int)$id ?>" class="btn btn-sm btn-secondary">Szerkeszt</a></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
...
<div class="card">
    <h2>Kontaktok</h2>
    <p><a href="<?= h(BASE_URL) ?>/organizers/kontakt_hozzaad.php?szervezo_id=<?= $id ?>" class="btn btn-primary btn-sm">Kontakt hozzáadása</a></p>
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?= h($k['email'] ?? '') ?></td>
                <td><?= h($k['telefon'] ?? '') ?></td>
                <td><a href="<?= h(BASE_URL) ?>/organizers/kontakt_levetel.php?szervezo_id=<?= $id ?>&kontakt_id=<?= (int)$k['id'] ?>" class="btn btn-sm btn-secondary">Lecsatolás</a></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
...
<div class="card">
    ...
    <p><a href="<?= h(BASE_URL) ?>/finance/cimek/letrehoz.php?szervezo_id=<?= $id ?>" class="btn btn-primary btn-sm">Új cím</a></p>
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><a href="<?= h(BASE_URL) ?>/finance/cimek/szerkeszt.php?id=<?= (int)$c['id'] ?>" class="btn btn-sm btn-secondary">Szerkeszt</a></td>
