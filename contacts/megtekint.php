...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['megjegyzes_szoveg'])) {
    ...
    if ($szoveg !== '') {
        ...
        redirect(BASE_URL . '/contacts/megtekint.php?id=' . $id . '#megjegyzesek');
    }
}
...
<div class="card">
    <h2><?= h($k['név']) ?></h2>
    <p>
        <a href="<?= h(BASE_URL) ?>/contacts/szerkeszt.php?id=<?= $id ?>" class="btn btn-primary">Szerkesztés</a>
        <a href="<?= h(BASE_URL) ?>/contacts/" class="btn btn-secondary">← Lista</a>
    </p>
    <table>
...
<div class="card">
    ...
    <ul>
        <?php foreach ($szervezok as $sz): ?>
            <li><a href="<?= h(BASE_URL) ?>/organizers/megtekint.php?id=<?= (int)$sz['id'] ?>"><?= h($sz['név']) ?></a></li>
        <?php endforeach; ?>
    </ul>
