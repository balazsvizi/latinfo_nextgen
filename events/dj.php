...
if ($djId <= 0) {
    ...
    echo events_public_dj_not_found_html($lang);
    exit;
}

$db = getDb();
$st = $db->prepare('SELECT `id`, `name` FROM `events_djs` WHERE `id` = ? LIMIT 1');
$st->execute([$djId]);
...
$eventsList = events_public_dj_published_events($db, $djId, events_public_post_status());
...
$title = $djName !== '' ? $djName : ('#' . $djId);
$desc = $lang === 'en'
    ? ($djName !== '' ? 'Published events with ' . $djName . ' on Latinfo.hu.' : 'Published events on Latinfo.hu.')
    : ($djName !== '' ? $djName . ' fellépései a Latinfo.hu-n.' : 'Közzétett események a Latinfo.hu-n.');

$canonical = events_absolute_url(events_url('dj.php?id=' . $djId));
$ogPageUrl = events_absolute_url(events_public_dj_page_url($djId, $lang));
$cssUrl = events_url('assets/event_public.css');
$urlHu = events_public_dj_lang_switch_url($djId, 'hu');
$urlEn = events_public_dj_lang_switch_url($djId, 'en');
...
<article class="event-public organizer-public">
    <header class="event-public__hero">
        <div class="event-public__hero-inner">
            <p class="event-public__eyebrow"><?= h($D['eyebrow']) ?></p>
            <h1 class="event-public__title"><?= h($title) ?></h1>
        </div>
    </header>

    <section class="organizer-public__events" aria-labelledby="dj-events-heading">
        <h2 class="organizer-public__events-title" id="dj-events-heading"><?= h($D['events_heading']) ?></h2>
        <?php if ($eventsList === []): ?>
            <p class="organizer-public__empty"><?= h($D['list_empty']) ?></p>
        <?php else: ?>
            <?php
            $djEventBlocks = [
                ['id' => 'dj-upcoming', 'heading' => $D['section_upcoming'], 'rows' => $eventsUpcoming, 'empty' => $D['upcoming_empty']],
                ['id' => 'dj-past', 'heading' => $D['section_past'], 'rows' => $eventsPast, 'empty' => $D['past_empty']],
            ];
            ?>
            <?php foreach ($djEventBlocks as $block): ?>
                <?php
                $isPastSection = ($block['id'] ?? '') === 'dj-past';
