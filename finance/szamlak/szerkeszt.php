...
if (!$id) {
    ...
    redirect(BASE_URL . '/finance/szamlak/');
}
$db = getDb();
...
$szerkeszt_url = BASE_URL . '/finance/szamlak/szerkeszt.php?id=' . $id;
if ($vissza !== '') {
...
if ($vissza === 'szervezo' && $vissza_szervezo_id > 0) {
    $redirect_after_delete = BASE_URL . '/organizers/megtekint.php?id=' . $vissza_szervezo_id;
} elseif ($vissza === 'szamlazando') {
    $redirect_after_delete = BASE_URL . '/finance/szamlazando/';
} else {
    $redirect_after_delete = BASE_URL . '/finance/szamlak/';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['torol_szamla'])) {
    ...
    if ($r === 'szervezo' && $rsz > 0) {
        redirect(BASE_URL . '/organizers/megtekint.php?id=' . $rsz);
    }
    if ($r === 'szamlazando') {
        redirect(BASE_URL . '/finance/szamlazando/');
    }
    redirect(BASE_URL . '/finance/szamlak/');
}
...
<div class="card">
    ...
    <p><a href="<?= h(BASE_URL) ?>/organizers/megtekint.php?id=<?= (int)$szamla['szervező_id'] ?>">← <?= h($szamla['szervezo_nev']) ?></a></p>
    <form method="post">
        ...
        <div class="form-actions">
            ...
            <a href="<?= h(BASE_URL) ?>/finance/szamlak/" class="btn btn-secondary">Vissza a listához</a>
        </div>
    </form>
...
<div class="card">
    ...
    <ul>
        ...
        <li>
            <a href="<?= h(BASE_URL) ?>/finance/szamlak/letoltes.php?id=<?= (int)$f['id'] ?>"><?= h($f['eredeti_név']) ?></a>
...
<div class="card">
    ...
    <ul class="szamlazando-csatolt-lista">
        ...
        <li class="szamlazando-csatolt-item">
            ...
            <a href="<?= h(BASE_URL) ?>/finance/szamlazando/szerkeszt.php?id=<?= (int)$sz['id'] ?>&vissza=szamla&szamla_id=<?= (int)$id ?>" class="btn btn-sm btn-secondary">Időszakok szerkesztése</a>
